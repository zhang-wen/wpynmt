...
def encode(text):
  ...
  for pos in xrange(1, len(text)):
    if is_alnum[pos] != is_alnum[pos - 1]:
      ...
      if token != u" " or token_start == 0: # append tokens splitted by space
        ret.append(token)
      token_start = pos
...
def decode(tokens):
  ...
  Args:
    ...
    '不_能_再_便宜_点_吗_ ?_' ->
    [u'不', u'能', u'再', u'便宜', u'点', u'吗', u' ?'] ->
    [u'不', u' ', u'能', u' ', u'再', u' ', u'便宜', u' ', u'点', u' ', u'吗', u' ?']
    => 不 能 再 便宜 点 吗 ?
  Returns:
    a unicode string
  """
  #print('into tokenizer decoder: ', tokens)
  token_is_alnum = [t[0] in _ALPHANUMERIC_CHAR_SET for t in tokens]
  ret = []
  for i, token in enumerate(tokens):
    if i > 0 and token_is_alnum[i - 1] and token_is_alnum[i]:
      # if token_is_alnum[i - 1] is False, token_is_alnum[i - 1] is space
      # space is not in _ALPHANUMERIC_CHAR_SET
      ret.append(u" ")
    ret.append(token)
    #print(ret)
  return "".join(ret)
...
def _read_filepattern(filepattern, max_lines=None, split_on_newlines=True):
  ...
  """
  filenames = sorted(glob(filepattern))
  lines_read = 0
...
def vocab_token_counts(text_filepattern, max_lines):
  ...
  for i, line in enumerate(
    ...
    if "," not in line:
      ("Malformed vocab line #%d '%s'", i, line)
      continue
