...
class Multihead_Additive_Attention(nn.Module):
    ...
    '''
        ...
        Args:
           k (FloatTensor): key vectors [key_len, batch_size, d_model]      ->  uh
           v (FloatTensor): value vectors [key_len, batch_size, 2*d_model]  ->  annotations
    ...
    def forward(self, q, v, k, attn_mask=None):
        ...
        key_len, batch_size = k.size(0), k.size(1)

        def reshape_head(x, nhead):
            return x.view(x.size(0), x.size(1), nhead, x.size(-1) // nhead).permute(1, 2, 0, 3)
        ...
        value_up = reshape_head(v, self.n_head)                   # [batch_size, n_head, key_len, dim_per_head]
        ...
        if attn_mask is not None:   # [key_len, batch_size]
        ...
        attn = alpha[:, :, :, None] * value_up      # [batch_size, n_head, key_len, dim_per_head]
        attn = unshape_head(attn, self.n_head)      # [batch_size, key_len, n_head * dim_per_head]
